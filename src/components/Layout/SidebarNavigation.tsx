...
export const SidebarNavigation: React.FC = () => {
  ...
  const renderMenuItem = (item: any) => {
      ...
      return <Collapsible key={item.path} defaultOpen={isActive || isSubmenuItemActive}>
          <SidebarMenuItem>
            <CollapsibleTrigger asChild>
              <SidebarMenuButton isActive={isActive || isSubmenuItemActive} className="hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus:bg-sidebar-accent focus:text-sidebar-accent-foreground focus:outline-none focus:ring-2 focus:ring-sidebar-ring transition-colors" aria-expanded={isActive || isSubmenuItemActive} aria-label={`${item.label} menu`}>
            ...
            <CollapsibleContent>
              <SidebarMenuSub>
                {item.submenu.map((subItem: any) => <SidebarMenuSubItem key={subItem.path}>
                    <SidebarMenuSubButton asChild isActive={location.pathname === subItem.path} className="focus:outline-none focus:ring-2 focus:ring-sidebar-ring transition-colors">
    ...
    return <SidebarMenuItem key={item.path}>
        <SidebarMenuButton asChild isActive={isActive} className="hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus:bg-sidebar-accent focus:text-sidebar-accent-foreground focus:outline-none focus:ring-2 focus:ring-sidebar-ring transition-colors">
  ...
  return <SidebarContent className="bg-[1f2124] bg-inherit">
      {/* Dashboard */}
      <SidebarGroup>
        <SidebarGroupContent>
          <SidebarMenu>
      ...
      {/* Timesheet Module */}
      {filteredTimesheetModule.length > 0 && <SidebarGroup>
      ...
      {filteredProjectModule.length > 0 && <SidebarGroup>
          <SidebarGroupLabel className="text-sidebar-foreground/60 text-xs font-medium px-2">
            Project Management Module
          </SidebarGroupLabel>
          <SidebarGroupContent>
      ...
      {filteredAdminModule.length > 0 && <SidebarGroup>
          <SidebarGroupLabel className="text-sidebar-foreground/60 text-xs font-medium px-2">
            Administrator Module
          </SidebarGroupLabel>
          <SidebarGroupContent>
