...
import { Users, Plus, Trash2, Calculator, CheckCircle2, XCircle, AlertTriangle, Copy, Download, Upload, Calendar, Building, Hash, Filter, UserPlus, UserMinus, Clock, Save, Eye, X, Zap, BarChart3 } from 'lucide-react';
...
export default function BulkTimesheetEntry() {
  ...
  const statistics = useMemo(() => {
    ...
    activeEmployees.forEach(emp => {
      ...
      dayKeys.forEach(day => {
        const dayData = dayHours[emp.employeeId]?.[day] || {
          regular: 0,
          overtime: 0
        };
  ...
  const filteredAvailableEmployees = useMemo(() => {
    return availableEmployees.filter(emp => classFilter === 'all' || emp.class === classFilter);
  }, [availableEmployees, classFilter]);
  return <div className="max-w-7xl mx-auto p-4 w-full px-0 py-0">
          ...
          <Card>
            <CardContent className="p-6">
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    <Select value="placeholder" onValueChange={addEmployee}>
                      ...
                      <SelectContent>
                        {filteredAvailableEmployees.map(emp => <SelectItem key={emp.employeeId} value={emp.employeeId}>
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                ...
                {Object.entries(quickTemplates).map(([key, template]) => <Button key={key} onClick={() => applyQuickTemplate(key as keyof typeof quickTemplates)} variant="outline" size="sm">
              ...
              <div>
                ...
                <Textarea value={bulkNotes} onChange={e => setBulkNotes(e.target.value)} placeholder="Enter notes to apply to all selected entries..." rows={2} className="resize-none" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-0">
              ...
              <div className="p-4 border-b bg-gray-50 sticky top-0 z-10">
                <div className="grid grid-cols-12 gap-2 items-center text-sm font-medium text-gray-700">
                  <div className="col-span-3 flex items-center space-x-2">
                    <Checkbox checked={selectedEmployees.size === activeEmployees.length && activeEmployees.length > 0} onCheckedChange={checked => checked ? selectAllEmployees() : clearSelection()} />
                    <span>Employee ({activeEmployees.length})</span>
                  </div>
                  {dayNames.map(day => <div key={day} className="text-center">
                      <div>{day}</div>
              ...
              <div className="max-h-96 overflow-y-auto">
                {activeEmployees.length === 0 ? <div className="p-8 text-center text-gray-500">
                ...
                return <div key={emp.employeeId} className={cn("border-b border-gray-100 p-4 hover:bg-gray-50 transition-colors", isSelected && "bg-blue-50 border-blue-200")}>
                        <div className="grid grid-cols-12 gap-2 items-center">
                          <div className="col-span-3 flex items-center space-x-2">
                            <Checkbox checked={isSelected} onCheckedChange={() => toggleEmployeeSelection(emp.employeeId)} />
                      ...
                      return <div key={day} className="text-center space-y-1">
                                {/* Regular Hours Input */}
                                <Input type="number" step="0.25" min="0" max="24" placeholder="0" value={dayData.regular || ''} onChange={e => updateHours(emp.employeeId, day, 'regular', e.target.value)} className={cn("w-16 h-6 text-xs text-center border-gray-300", totalDayHours > 16 && "border-red-500 bg-red-50", totalDayHours > 12 && totalDayHours <= 16 && "border-yellow-500 bg-yellow-50")} title="Regular Hours" />
                                {/* Overtime Hours Input */}
                                <Input type="number" step="0.25" min="0" max="24" placeholder="0" value={dayData.overtime || ''} onChange={e => updateHours(emp.employeeId, day, 'overtime', e.target.value)} className={cn("w-16 h-6 text-xs text-center border-orange-300 bg-orange-50/30", totalDayHours > 16 && "border-red-500 bg-red-50")} title="Overtime Hours" />
                              </div>;
                    })}
                          
                          <div className="text-center">
                            <div className={cn("font-medium text-xs", grandTotal > 48 ? "text-red-600" : grandTotal > 40 ? "text-yellow-600" : "text-gray-900")}>
                          ...
                          <div className="text-center flex justify-center space-x-1">
                            <Button variant="ghost" size="sm" onClick={() => copyRowToSelected(emp.employeeId)} className="h-6 w-6 p-0" title="Copy to selected employees">
                              <Copy className="w-3 h-3" />
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => removeEmployee(emp.employeeId)} className="h-6 w-6 p-0 text-red-600 hover:text-red-700" title="Remove employee">
          ...
          {validationErrors.length > 0 && <Alert variant="destructive">
              ...
              <AlertDescription>
                ...
                <ul className="text-sm space-y-1">
                  {validationErrors.slice(0, 5).map((error, index) => <li key={index}>• {error.message}</li>)}
            ...
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-3">
                  <Button onClick={() => setShowPreview(true)} variant="outline" disabled={statistics.validEntries === 0}>
                    <Eye className="w-4 h-4 mr-1" />
                    Preview
                  </Button>
                  <Button onClick={submitEntries} disabled={isSubmitting || statistics.validEntries === 0 || validationErrors.length > 0} className="min-w-[120px]">
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="space-y-3">
                {Object.entries(quickTemplates).map(([key, template]) => <Button key={key} variant="outline" className="w-full justify-start" onClick={() => applyQuickTemplate(key as keyof typeof quickTemplates)} disabled={selectedEmployees.size === 0}>
            ...
            <CardContent className="p-4">
              ...
              <ScrollArea className="h-48">
                <div className="space-y-2">
                  {filteredAvailableEmployees.map(emp => <div key={emp.employeeId} className="flex items-center justify-between text-sm">
                      <div className="flex-1 min-w-0">
                        <div className="font-medium truncate">{emp.fullName}</div>
                        <div className="text-xs text-gray-500">{emp.employeeId} - {emp.class}</div>
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => addEmployee(emp.employeeId)} className="h-6 w-6 p-0">
                        <Plus className="w-3 h-3" />
                      </Button>
                    </div>)}
                  {filteredAvailableEmployees.length === 0 && <div className="text-center text-gray-500 text-sm py-4">
      ...
      {/* Preview Modal */}
      {showPreview && <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
              ...
              <div className="space-y-4">
                ...
                <ScrollArea className="h-64">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {collectEntries().map((entry, index) => <tr key={index} className="border-t border-gray-100">
