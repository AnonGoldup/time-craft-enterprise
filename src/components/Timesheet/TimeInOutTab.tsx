...
export const TimeInOutTab: React.FC<TabContentProps> = ({
  ...
  const employees: Employee[] = mockEmployees.filter(emp => emp.id && emp.name) // Filter first to ensure we have required data
  .map(emp => ({
    employeeId: emp.id!,
    fullName: emp.name!,
  ...
  return <CardContent className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          <Button type="button" variant="outline" size="sm" onClick={addNewEntry} className="flex items-center space-x-1">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        {entries.map((entry, index) => <div key={index} className="border rounded-lg p-4 relative">
            {entries.length > 1 && <div className="absolute top-2 right-2">
                <Button type="button" variant="ghost" size="sm" onClick={() => removeEntry(index)} className="h-6 w-6 p-0 text-red-600 hover:text-red-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <MultiEmployeeSelector employees={employees.filter(emp => emp.isActive !== false)} selectedEmployees={employees.filter(emp => emp.employeeId === entry.employeeId)} onEmployeeChange={selected => {
              if (selected.length > 0) {
                handleInputChange(index, 'employeeId', selected[0].employeeId);
              }
            }} placeholder="Select employee..." maxSelected={1} disabled={false} />
              </div>
              <div>
                <Label>Date Worked *</Label>
                <MultiDatePicker selectedDates={entry.dateWorked ? [new Date(entry.dateWorked)] : []} onDateChange={dates => {
              if (dates.length > 0) {
                handleInputChange(index, 'dateWorked', dates[0].toISOString().split('T')[0]);
              }
            }} placeholder="Select date..." maxDates={1} />
              </div>
            </div>
            ...
            <div className="border-t pt-6 mb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <Select value={entry.projectCode} onValueChange={value => handleInputChange(index, 'projectCode', value)}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select Project" />
                    </SelectTrigger>
                    <SelectContent>
                      {projects.map(project => <SelectItem key={project.code} value={project.code}>
                ...
                <div>
                  <Label>Extra</Label>
                  <Select value={entry.extraValue} onValueChange={value => handleInputChange(index, 'extraValue', value)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {extras.map(extra => <SelectItem key={extra.extraID} value={extra.extraValue}>
              ...
              <div className="mb-4">
                ...
                <Select value={entry.costCode} onValueChange={value => handleInputChange(index, 'costCode', value)}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select Cost Code" />
                  </SelectTrigger>
                  <SelectContent>
                    {costCodes.map(code => <SelectItem key={code.costCodeID} value={code.costCode}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              <div>
                <Label>Time In *</Label>
                <Input type="time" value={entry.timeIn} onChange={e => {
              ...
              <div>
                ...
                <Input type="time" value={entry.timeOut} onChange={e => {
              ...
              <div>
                ...
                <Input type="time" value={entry.breakStart} onChange={e => {
              ...
              <div>
                ...
                <Input type="time" value={entry.breakEnd} onChange={e => {
            ...
            {crossesMidnight && <div className="bg-amber-50 border border-amber-200 p-4 rounded-lg border-l-4 border-l-amber-500 mb-4">
            ...
            <div>
              <Label>Notes</Label>
              <Textarea rows={3} placeholder="Enter any additional notes..." value={entry.notes} onChange={e => handleInputChange(index, 'notes', e.target.value)} />
            </div>
          </div>)}
