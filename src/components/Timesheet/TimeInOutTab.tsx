...
const validateEntry = (entry: TimeEntry): ValidationError[] => {
  ...
  if (!entry.dateWorked) {
    errors.push({ field: 'dateWorked', message: 'Date is required' });
  } else if (!validateFutureDate(entry.dateWorked)) {
    errors.push({ field: 'dateWorked', message: 'Cannot enter time for future dates' });
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(`Successfully created ${timesheetEntries.length} timesheet entries!\n\nEntries:\n${timesheetEntries.map((e, i) => 
        `${i+1}. ${e.employeeName} (${e.employeeId}) - ${e.dateWorked} - ${e.standardHours + e.overtimeHours}hrs (Project: ${e.projectCode})`
      ).join('\n')}`);
              ...
              return (
                <div key={entry.id} className="border rounded-lg p-3 relative space-y-3">
                  ...
                  <div className="mb-2">
                    ...
                    {entry.selectedEmployees.length > 0 && (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      {getFieldError(entry.id, 'dateWorked') && (
                        <p className="text-xs text-red-600 mt-1">{getFieldError(entry.id, 'dateWorked')}</p>
                      )}
                    </div>
                  </div>
                  ...
                  {calc && (
                    <div className="bg-muted p-2 rounded-lg">
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        {entry.selectedEmployees.length > 1 && (
                          <Badge variant="outline" className="text-xs">
                            × {entry.selectedEmployees.length} employees
                          </Badge>
                        )}
