...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isFirstTime && !email) {
      toast.error('Please enter your email for first-time setup');
      return;
    }
    ...
    try {
      ...
      if (isFirstTime) {
        // First-time login with email
        await login(employeeId, email);
      } else {
      ...
      if (error.needsPasswordSetup) {
        ...
        window.location.href = `/setup-password?token=${error.setupToken}`;
      } else {
        ...
        <CardContent className="space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="employeeId" className="text-white/90 font-medium">Employee ID</Label>
              <Input
                id="employeeId"
                type="text"
                value={employeeId}
            ...
            {!isFirstTime ? (
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    className="bg-white/5 border-white/10 focus:border-orange-400/50 focus:ring-orange-400/20 text-white placeholder:text-white/50 backdrop-blur-sm h-12 pr-10"
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="bg-white/5 border-white/10 focus:border-orange-400/50 focus:ring-orange-400/20 text-white placeholder:text-white/50 backdrop-blur-sm h-12"
                  placeholder="john.doe@altapro.com"
                  disabled={loading}
                />
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {isFirstTime ? 'I have a password' : 'First time user?'}
              </button>
            </div>
          ...
          <div className="p-4 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                <p className="text-orange-300 font-medium mb-2">Login Instructions:</p>
                <div className="space-y-1 text-white/70">
                  {isFirstTime ? (
                    <>
                      <p>• Use your Employee ID and company Email</p>
                      <p>• You'll be prompted to create a password</p>
                    </>
                  ) : (
                    <>
                      <p>• Use your Employee ID and Password</p>
                      <p>• Click "First time user?" if you need to set up your password</p>
                    </>
                  )}
                </div>
              </div>
